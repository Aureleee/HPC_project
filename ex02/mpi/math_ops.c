...
/* same idea as your previous chunk compute, just writing into y_local */
                       ...
                       double *y_local, int start, int count)
{
    ...
    {
        int g = start + i;
        double sum = diag->vec[g] * b_full->vec[g];

        if (g > 0)
            sum += lower->vec[g - 1] * b_full->vec[g - 1];
        if (g < n - 1)
            sum += upper->vec[g] * b_full->vec[g + 1];
